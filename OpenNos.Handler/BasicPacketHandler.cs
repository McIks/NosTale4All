...
namespace OpenNos.Handler
{
    public class BasicPacketHandler : IPacketHandler
    {
        ...
        {
            // Needed for $TargetInfo
            // {
            Session.Character.LastTargetId = ncifPacket.TargetId;
            Session.Character.LastTargetType = ncifPacket.Type;
            // }
        ...
        public void StartGame(GameStartPacket gameStartPacket)
        {
            ...
            if (ServerManager.Instance.Configuration.WorldInformation)
            {
                ...
                Session.SendPacket(Session.Character.GenerateSay("Discord: https://discord.gg/NSfm2uw", 12));
...
#pragma warning restore 618
            ...
            {
                Session.SendPacket(UserInterfaceHelper.Instance.GenerateModal("Welcome to NosTale4All Adventurer! Join our Discord to get the lastest news and events! Enjoy ;)", 1));
        ...
        public void Walk(WalkPacket walkPacket)
        {
            if (!Session.Character.NoMove)
            {
                ...
                if (Session.HasCurrentMapInstance && !Session.CurrentMapInstance.Map.IsBlockedZone(walkPacket.XCoordinate, walkPacket.YCoordinate) && !Session.Character.IsChangingMapInstance && !Session.Character.HasShopOpened)
                {
                    ...
                    {
                        Session.SendPacket(UserInterfaceHelper.Instance.GenerateInfo("Nos4All doesn't need cheaters!"));
                    }
                }
            }
        }
